...
/* DRrectCompRing() -
| Test if data rectangle intersects the shaded area between outer rectangle
| and inner rectangle
*/
bool_t DRrectCompRing(
	int rfd,       /* DR index file descriptor */
	DRrect *outerrect,   /* Outer rectangle */
	DRrect *innerrect,   /* Inner rectangle */
	DRrect *datarect)    /* Data rectangle */
{
	if (DRrectOverlap(rfd, outerrect, datarect) && 
	    !DRrectCover(rfd, innerrect, datarect))
		return TRUE;
...
/* DRrectPrintOneLine() -
|
|	Print a rect in one line
+-----------------------------------------------------------------------------*/
void DRrectPrintOneLine(
	int	rfd,		/* DR index file descriptor */
	DRrect	*rect)		/* rect to print */
{
	int	l;		/* low-side number of rect */
