...
|	all leaf entries whose cover rectangles intersect the shaded area between
|   the outer rectangle and inner rectangle.
...
{
	bool_t (*testfn)();  /* comparsion function */
	int (*outputfn)(); /* rect output function */
	...
	/* check "testrect" */
	if (DRrectInvalid(rfd, outerrect) || DRrectInvalid(rfd, innerrect))
		return DRerrSave(DRE_RECTINVALID);
	
	/* initialize "testfn" */
	testfn = DRrectCompRing;

	/* initialize "outputfn" */
	outputfn = DRnodePrintRing;

	/* search index tree */
	return DRtreeRingSearch(rfd, DRrootPage(rfd), outerrect, innerrect,
						testfn, outputfn);
}
